import { mountUniversalLookupTheme } from '../shared/universalLookupTheme.js';
...
mountUniversalLookupTheme(document);
...
class InteractiveSubtitles {
  ...
  setupArticleSelectionMode() {
    ...
    document.addEventListener('mouseup', (e) => {
      ...
      if (
        e.target.closest('.definition-modal') ||
        e.target.closest('.interactive-sidebar-container') ||
        e.target.closest('#interactive-subtitle-container') ||
        e.target.closest('.article-selection-hint')
      ) {
        return;
      }
      ...
      // Use the selected text itself as the context sentence shown in the UI.
      // extractContextFromSelection expands it to the surrounding block if possible.
      const context = extractContextFromSelection(selection, selectedText);

      // Clear the page selection so it doesn't linger visually
      selection.removeAllRanges();

      // Open the universal UI immediately.
      // The UI's interactive sentence area lets the user select the specific
      // word(s) they want to look up — word picking happens INSIDE the UI.
      if (this.displayMode === 'sidebar') {
        if (!this.sidebarContainer) this.createSidebarContainer();
        this.showDefinitionInSidebar(selectedText, context, null);
      } else {
        this.showDefinitionModal(selectedText, context, null);
      }

      console.log('[ArticleMode] Opened UI for context:', context.substring(0, 80));
    });
  ...
  loadSettings() {
    return new Promise((resolve) => {
      chrome.storage.sync.get(['displayMode', 'ankiDeck', 'ankiNoteType', 'disabledSites', 'aiProvider', 'customLlmUrl', 'customLlmApiKey', 'customLlmModel', 'mistralApiKey', 'languageMappings', 'ankiDeckItalian'], (result) => {
        ...
        if (defaultDisabled.some(site => currentHost.includes(site))) {
          this.enabled = false;
  ...
  createSidebarContainer() {
    const existingSidebar = document.getElementById('interactive-sidebar-container');
    if (existingSidebar) existingSidebar.remove();

    this.sidebarContainer = document.createElement('div');
    this.sidebarContainer.id = 'interactive-sidebar-container';
    this.sidebarContainer.className = 'interactive-sidebar-container';
    this.sidebarContainer.style.display = 'none';
    this.sidebarContainer.innerHTML = `
      <div class="sidebar-header" style="cursor: move;">
        <h3 style="flex: 1; margin: 0;">📖 Word Lookup</h3>
        <div class="sidebar-controls" style="display: flex; gap: 4px; align-items: center;">
          <button class="sidebar-theme-btn" title="Toggle Theme" style="background:none;border:none;cursor:pointer;font-size:16px;padding:2px 4px;">🌙</button>
          <button class="sidebar-text-minus-btn" title="Decrease text size" style="background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px;font-weight:bold;color:inherit;">A-</button>
          <button class="sidebar-text-plus-btn" title="Increase text size" style="background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px;font-weight:bold;color:inherit;">A+</button>
          <button class="sidebar-close-btn" style="color:inherit;">&times;</button>
        </div>
      </div>
    ...
    this.sidebarContainer.querySelector('.sidebar-close-btn').addEventListener('click', () => {
      this.sidebarContainer.style.display = 'none';
    });

    // Theme Toggle
    const themeBtn = this.sidebarContainer.querySelector('.sidebar-theme-btn');
    themeBtn.addEventListener('click', () => {
      this.sidebarContainer.classList.toggle('dark-mode');
      themeBtn.textContent = this.sidebarContainer.classList.contains('dark-mode') ? '☀️' : '🌙';
    });

    // Text Resize
    const contentArea = this.sidebarContainer.querySelector('.sidebar-content');
    let currentFontSize = 14;
    contentArea.style.fontSize = currentFontSize + 'px';
    this.sidebarContainer.querySelector('.sidebar-text-plus-btn').addEventListener('click', () => {
      currentFontSize = Math.min(currentFontSize + 2, 24);
      contentArea.style.fontSize = currentFontSize + 'px';
    });
    this.sidebarContainer.querySelector('.sidebar-text-minus-btn').addEventListener('click', () => {
      currentFontSize = Math.max(currentFontSize - 2, 10);
      contentArea.style.fontSize = currentFontSize + 'px';
    });

    // Dragging Logic
    const header = this.sidebarContainer.querySelector('.sidebar-header');
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      this.sidebarContainer.style.right = (initialRight + dx) + 'px';
      this.sidebarContainer.style.top = (initialTop + dy) + 'px';
    });
  ...
  renderInteractiveSubtitle(subtitle) {
    ...
    subtitleText.addEventListener('dblclick', (e) => {
      this._subtitleDragged = false;
      ...
      if (startWordEl && endWordEl) {
        ...
        if (phrase.length > 0) {
          ...
          if (container.classList.contains('sentence-lookup-area')) {
            ...
            // Auto click AI button to search immediately
  ...
  handleWordClick(word, element) {
    ...
    if (this.displayMode === 'sidebar') {
      this.showDefinitionInSidebar(cleanWord, context, element);
    } else {
  ...
  buildDictionaryLinksMarkup(word, isItalian, variant = 'modal') {
    if (isItalian) {
      if (variant === 'sidebar') {
        return `
  ...
  updateLookupTarget(targetRoot, wordOrPhrase, context) {
    ...
    const isSidebar = !!targetRoot.querySelector('.sidebar-tabs');
    ...
    const sidebarTitle = targetRoot.querySelector('.sidebar-word-header h3');
    if (modalTitle) modalTitle.textContent = `Definition: "${normalizedWord}"`;
    if (sidebarTitle) sidebarTitle.textContent = `Definition: "${normalizedWord}"`;

    const tabBtn = targetRoot.querySelector('[data-tab="merriam"]');
    if (tabBtn) tabBtn.textContent = websterTabName;

    const badge = targetRoot.querySelector('#modal-target-word-badge') || targetRoot.querySelector('#sidebar-target-word-badge');
    if (badge) badge.textContent = normalizedWord;

    const aiBtn = targetRoot.querySelector('#get-ai-definition') || targetRoot.querySelector('#sidebar-get-ai');
    if (aiBtn) {
    ...
    const merriamFrame = isSidebar
      ? targetRoot.querySelector('.sidebar-panel[data-tab="merriam"] iframe')
      : targetRoot.querySelector('.tab-content[data-tab="merriam"] iframe');
    const merriamLink = isSidebar
      ? targetRoot.querySelector('.sidebar-panel[data-tab="merriam"] .sidebar-external-link')
      : targetRoot.querySelector('.tab-content[data-tab="merriam"] .external-link');

    if (merriamFrame) merriamFrame.src = merriamUrl;
    if (merriamLink) merriamLink.href = merriamUrl;

    const wikiFrame = isSidebar
      ? targetRoot.querySelector('.sidebar-panel[data-tab="wikipedia"] iframe')
      : targetRoot.querySelector('.tab-content[data-tab="wikipedia"] iframe');
    const wikiLink = isSidebar
      ? targetRoot.querySelector('.sidebar-panel[data-tab="wikipedia"] .sidebar-external-link')
      : targetRoot.querySelector('.tab-content[data-tab="wikipedia"] .external-link');

    if (wikiFrame) wikiFrame.src = wikiUrl;
    if (wikiLink) wikiLink.href = wikiUrl;

    if (isSidebar) {
      const dictContainer = targetRoot.querySelector('.sidebar-dict-links');
      if (dictContainer) {
        dictContainer.innerHTML = this.buildDictionaryLinksMarkup(normalizedWord, isItalian, 'sidebar');
      }
    } else {
      const dictContainer = targetRoot.querySelector('.tab-content[data-tab="dictionaries"] .dictionary-links');
      if (dictContainer) {
        dictContainer.innerHTML = this.buildDictionaryLinksMarkup(normalizedWord, isItalian, 'modal');
      }
    }
  }

  showDefinitionInSidebar(word, context, clickedElement) {
    if (!this.sidebarContainer) this.createSidebarContainer();
    const contentArea = this.sidebarContainer.querySelector('.sidebar-content');
    const isItalian = this.isItalianLang(word, context);
    const websterTabName = isItalian ? 'Wiktionary IT' : 'Merriam-Webster';

    // Build the same tab structure as the overlay modal, but vertically
    contentArea.innerHTML = `
      <div class="sidebar-word-header">
        <h3>Definition: "${word}"</h3>
      </div>
      <div class="sidebar-tabs">
        <button class="sidebar-tab-btn active" data-tab="ai">AI Context</button>
        <button class="sidebar-tab-btn" data-tab="merriam">${websterTabName}</button>
        <button class="sidebar-tab-btn" data-tab="wikipedia">Wikipedia</button>
        <button class="sidebar-tab-btn" data-tab="dictionaries">Other Dictionaries</button>
      </div>
      <div class="sidebar-tab-panels">
        <div class="sidebar-panel active" data-tab="ai">
          <div class="sidebar-ai-section">
            <div class="context-display" style="padding: 12px; background: inherit; border: 1px solid #e9ecef; border-radius: 6px; margin-bottom: 12px;">
              <div style="margin-bottom: 10px;">
                <strong style="font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em; color: #5f6368;">Target word(s):</strong>&nbsp;
                <span id="sidebar-target-word-badge" style="background: rgba(59, 130, 246, 0.2); color: #3b82f6; padding: 3px 10px; border-radius: 12px; font-weight: bold; font-size: 15px;">${word}</span>
              </div>
              <strong>Context Sentence:</strong><br>
              <div id="sidebar-interactive-sentence-container"
                   class="sentence-lookup-area"
                   style="margin: 10px 0; font-size: 15px; line-height: 1.6; color: inherit;
            ...
            <button id="sidebar-get-ai" class="ai-btn" data-target-word="${word}" style="width:100%; margin-top: 10px;">Get AI Definition for "${word}"</button>
            <div id="sidebar-ai-response" class="ai-response"></div>
            <div id="sidebar-ai-chat" class="ai-chat-container" style="display:none; margin-top: 16px; border-top: 1px solid #e9ecef; padding-top: 16px;">
              <div class="ai-chat-messages" id="sidebar-chat-messages" style="max-height: 200px; overflow-y: auto; margin-bottom: 8px;"></div>
              <div class="ai-chat-input-area" style="display: flex; gap: 8px;">
                <input type="text" class="ai-chat-input" id="sidebar-chat-input" placeholder="Ask a follow-up question..." style="flex:1;">
                <button class="ai-chat-send-btn" id="sidebar-chat-send" title="Send">➤</button>
              </div>
            </div>
          </div>
        </div>
        <div class="sidebar-panel" data-tab="merriam">
          <div class="sidebar-iframe-wrap">
            <iframe src="${isItalian ? `https://en.wiktionary.org/wiki/${encodeURIComponent(word.toLowerCase())}#Italian` : `https://www.merriam-webster.com/dictionary/${encodeURIComponent(word)}`}" sandbox="allow-same-origin allow-scripts allow-popups allow-forms" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
            <a href="${isItalian ? `https://en.wiktionary.org/wiki/${encodeURIComponent(word.toLowerCase())}#Italian` : `https://www.merriam-webster.com/dictionary/${encodeURIComponent(word)}`}" target="_blank" class="sidebar-external-link">Open in New Tab →</a>
          </div>
        </div>
        <div class="sidebar-panel" data-tab="wikipedia">
          <div class="sidebar-iframe-wrap">
            <iframe src="https://en.wikipedia.org/wiki/${encodeURIComponent(word)}" sandbox="allow-same-origin allow-scripts allow-popups allow-forms" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
            <a href="https://en.wikipedia.org/wiki/${encodeURIComponent(word)}" target="_blank" class="sidebar-external-link">Open in New Tab →</a>
          </div>
        </div>
        <div class="sidebar-panel" data-tab="dictionaries">
          <div class="sidebar-dict-links">
            ${this.buildDictionaryLinksMarkup(word, isItalian, 'sidebar')}
          </div>
        </div>
      </div>
      <div class="sidebar-anki-bar">
        <label style="display:flex;align-items:center;gap:4px;color:#5f6368;font-size:11px;cursor:pointer;user-select:none;">
          <input type="checkbox" id="sidebar-screenshot-cb" checked style="accent-color:#3b82f6;cursor:pointer;"> Screenshot
        </label>
        <button id="sidebar-anki-btn" class="anki-btn" style="flex:1;">⭐ Add to Anki</button>
        <span id="sidebar-anki-status" class="anki-status" style="font-size:11px;color:#6c757d;"></span>
      </div>
    `;

    setTimeout(() => {
        const interactiveContainer = contentArea.querySelector('#sidebar-interactive-sentence-container');
        if (interactiveContainer) {
            this.renderInteractiveSentence(context, interactiveContainer, contentArea);
        }
    }, 0);

    // Tab switching
    contentArea.querySelectorAll('.sidebar-tab-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        contentArea.querySelectorAll('.sidebar-tab-btn').forEach(b => b.classList.remove('active'));
        e.target.classList.add('active');
        const tabName = e.target.getAttribute('data-tab');
        contentArea.querySelectorAll('.sidebar-panel').forEach(p => {
          p.classList.remove('active');
          p.style.display = 'none';
        });
        const activePanel = contentArea.querySelector(`.sidebar-panel[data-tab="${tabName}"]`);
        if (activePanel) {
    ...
    // Show active panel
    contentArea.querySelectorAll('.sidebar-panel').forEach(p => p.style.display = 'none');
    contentArea.querySelector('.sidebar-panel.active').style.display = 'block';

    // AI button
    const sidebarAiBtn = contentArea.querySelector('#sidebar-get-ai');
    sidebarAiBtn.addEventListener('click', () => {
      const responseDiv = contentArea.querySelector('#sidebar-ai-response');
      const targetWord = sidebarAiBtn.getAttribute('data-target-word') || word;
      // Show loading state on button and response area
      sidebarAiBtn.disabled = true;
      sidebarAiBtn.textContent = '⏳ Getting AI definition...';
      sidebarAiBtn.style.opacity = '0.6';
      responseDiv.innerHTML = '<div class="loading">Getting AI definition...</div>';
      this.conversationHistory = [];
      this.getMistralDefinition(targetWord, context, responseDiv, () => {
        // Restore button state
        sidebarAiBtn.disabled = false;
        const currentWord = sidebarAiBtn.getAttribute('data-target-word') || word;
        sidebarAiBtn.textContent = 'Get AI Definition for "' + currentWord + '"';
        sidebarAiBtn.style.opacity = '1';
        // Show chat container after AI response
        const chatContainer = contentArea.querySelector('#sidebar-ai-chat');
        if (chatContainer) chatContainer.style.display = 'block';
      });
    });

    // Interactive sentence selection handled by renderInteractiveSentence in showDefinitionInSidebar

    // Sidebar Context Editing
    const editBtn = contentArea.querySelector('.edit-context-btn');
    const saveBtn = contentArea.querySelector('.save-context-btn');
    const cancelBtn = contentArea.querySelector('.cancel-context-btn');
    const displayWrap = contentArea.querySelector('.context-edit-wrapper');
    const editorWrap = contentArea.querySelector('.context-editor');
    const textarea = contentArea.querySelector('.context-textarea');
    const textSpan = contentArea.querySelector('.context-text');
    ...
    if (saveBtn) {
      saveBtn.addEventListener('click', () => {
        ...
        if (newContext) {
          context = newContext; // Update local context variable
          textSpan.textContent = `"${newContext}"`;
          displayWrap.style.display = 'block';
          editorWrap.style.display = 'none';
          console.log('[CONTEXT] Manually updated context:', newContext);
        }
      });
    }

    // Chat follow-up (sidebar)
    const chatSendBtn = contentArea.querySelector('#sidebar-chat-send');
    const chatInput = contentArea.querySelector('#sidebar-chat-input');
    if (chatSendBtn && chatInput) {
      const sendChat = () => {
        const message = chatInput.value.trim();
        if (!message) return;
        chatInput.value = '';
        const messagesDiv = contentArea.querySelector('#sidebar-chat-messages');
        let currentTargetWord = sidebarAiBtn ? (sidebarAiBtn.getAttribute('data-target-word') || word) : word;
        this.sendChatFollowUp(message, messagesDiv, currentTargetWord, context);
      };
    ...
    // Anki button
    contentArea.querySelector('#sidebar-anki-btn').addEventListener('click', async () => {
      const liveWord = (contentArea.querySelector('#sidebar-get-ai')?.getAttribute('data-target-word') || word).trim();
      await this.handleAddToAnki(liveWord, context, contentArea);
    });
  ...
  getArticleContext(word) {
    ...
    if (this.lastContextMenuContext && this.lastContextMenuContext.word.toLowerCase().includes(word.toLowerCase())) {
  ...
  showDefinitionModal(word, context, clickedElement) {
    ...
    modalWordHeader.innerHTML = `<h3 class="modal-word-title">Definition: "${word}"</h3>`;
    ...
    const merriamUrl = isItalian ? `https://en.wiktionary.org/wiki/${encodeURIComponent(word.toLowerCase())}#Italian` : `https://www.merriam-webster.com/dictionary/${encodeURIComponent(word)}`;
    const merriamContent = this.createIframeContent('merriam', merriamUrl);
    const dictionariesContent = this.createDictionariesContent(word, isItalian);
    const wikipediaContent = this.createIframeContent('wikipedia', `https://en.wikipedia.org/wiki/${encodeURIComponent(word)}`);
    const aiContent = this.createAIContent(word, context);
    ...
    ankiContainer.innerHTML = `
      <label style="display:flex;align-items:center;gap:4px;color:#6c757d;font-size:12px;cursor:pointer;user-select:none;">
        <input type="checkbox" id="modal-screenshot-cb" checked style="accent-color:#3b82f6;cursor:pointer;"> Screenshot
      </label>
  ...
  createDictionariesContent(word, isItalian) {
    ...
    container.innerHTML = `
      ...
      <div class="dictionary-links">
        ${this.buildDictionaryLinksMarkup(word, isItalian, 'modal')}
      </div>
  ...
  createAIContent(word, context) {
    ...
    aiContainer.innerHTML = `
      ...
      <div class="context-display">
        <div style="margin-bottom: 10px;">
          <strong style="font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em; color: #5f6368;">Target word(s):</strong>&nbsp;
          <span id="modal-target-word-badge" style="background: rgba(59, 130, 246, 0.2); color: #3b82f6; padding: 3px 10px; border-radius: 12px; font-weight: bold; font-size: 15px;">${word}</span>
        </div>
        <strong>Context Sentence:</strong><br>
      ...
      <div id="pdf-tip" style="display: ${word === context ? 'block' : 'none'}; margin-top: 10px; padding: 10px; background: #fff8e1; border-left: 4px solid #ffc107; border-radius: 4px; font-size: 12px; color: #856404;">
      </div>
      <button id="get-ai-definition" class="ai-btn" data-target-word="${word}">Get AI Definition for "${word}"</button>
  ...
  setupModalEventListeners(modal, word, context) {
    ...
    if (themeBtn) {
      themeBtn.addEventListener('click', () => {
        ...
        themeBtn.textContent = contentDiv.classList.contains('dark-mode') ? '☀️' : '🌙';
      });
    }
...
1. **Language Detection**: Identify the language of the word. If the language is one of [${configuredLanguagesStr}], please output exactly that language name.
2. **Part of Speech**: EXACTLY ONE of [noun, verb, adjective, adverb, pronoun, preposition, conjunction, interjection, expression].
3. **Phonetics & Stress**: The IPA pronunciation of the word (e.g. /fəˈnɛtɪks/).
4. **Meaning in Context**: The specific meaning of "${word}" as used in this particular video context.
5. **Alternative Meanings**: Other common meanings of this word.
6. **Example Sentences**: 2-3 example sentences using the word.
7. **Cultural & Linguistic Nuances**: Any cultural, idiomatic, or contextual nuances.
8. **Visual**: [Yes/No] - Is this word a concrete object, place, or person that can be visualized?
9. **Visual Query**: Two or three English keywords to search for an image (e.g. "red apple", "eiffel tower").

**If the word is an Italian verb**, additionally provide:
10. **Verb Conjugation Table** in this exact format:
...
Be clear and concise, focusing on how the word fits within this specific context.`;
...
Please provide:
1. Language Detection (If it is one of [${configuredLanguagesStr}], output exactly that name)
2. Phonetics & Stress
3. Meaning in Context
4. Alternative Meanings
5. Example Sentences (2-3)
6. Cultural & Linguistic Nuances
...
Be clear and concise.`;
  ...
  formatAIResponse(text, word, context) {
  ...
  extractMeaningFromAI(aiResponseText, word) {
    if (!aiResponseText) return '';
    const text = aiResponseText;
  ...
  // Extract conjugation table HTML from Mistral AI response (for Italian verbs)
  extractConjugationFromAI(aiResponseText) {
    if (!aiResponseText) return null;

    // Look for markdown table with conjugation data
    const tableMatch = aiResponseText.match(/\|[^\n]*(?:io|presente|Tense)[^\n]*\|[\s\S]*?(?:\n\n|\n(?:[^|])|\Z)/i);
    ...
    rows.forEach((row, idx) => {
      ...
      const bgColor = idx === 0 ? 'background:#2a2a3a;color:#e5e7eb;' : '';
      html += '<tr>';
      cells.forEach(cell => {
        html += `<${tag} style="border:1px solid #444;padding:3px 5px;${bgColor}">${cell.trim()}</${tag}>`;
      });
      html += '</tr>';
    });
    html += '</table>';
  ...
  // Extract an example sentence from the AI response text (different from the context)
  extractExampleSentence(aiResponseText, word) {
    ...
    for (const line of lines) {
      ...
      // Must contain the word and look like a proper sentence
      if (trimmed.toLowerCase().includes(wordLower) && /[.!?]$/.test(trimmed)) {
        examples.push(trimmed);
      }
    }

    // Return the first example that looks like a standalone sentence
  ...
  async sendToAnki(word, definition, context, screenshot) {
    ...
    const rawAiLower = (rawAIText || '').toLowerCase();
    ...
    const posInfo = this.detectPartOfSpeech(word, rawAIText || definition);
    const phonetics = this.extractPhoneticsFromAI(rawAIText || definition);

    let meaning = this.extractMeaningFromAI(rawAIText || definition, word);
    // Mask the word in the meaning to avoid spoilers
    if (meaning) {
      meaning = this.maskWordInText(meaning, word);
    }

    const conjugation = posInfo.isItalianVerb ? this.extractConjugationFromAI(rawAIText || definition) : null;
    const exampleSentence = this.extractExampleSentence(rawAIText, word);
    ...
    console.log('[ANKI-CONTENT] Example sentence:', exampleSentence);

    // POS abbreviation (n. / v. / adj. / adv.)
    const posAbbr = this.getPosAbbreviation(posInfo.pos);
    const posTag = posAbbr ? ` <i style="color:#999;">${posAbbr}</i>` : '';
    ...
    if (isCloze) {
      ...
      const phoneticsTag = phonetics ? ` <span style="font-family: Arial, sans-serif; font-weight: normal; color: #555; margin-left: 8px;">${phonetics}</span>` : '';
      backExtra += `<div style="font-size:1.2em;margin-bottom:8px;color:#2a2a3a;"><b>${word}</b>${phoneticsTag}${posTag}</div>`;

      // 2. Meaning
      const rawMeaning = this.extractMeaningFromAI(rawAIText || definition, word);
      ...
      // 3. Example
      if (exampleSentence) {
        backExtra += `<div style="margin-bottom:8px; font-style: italic; color: #555;">Example: ${exampleSentence}</div>`;
      }

      // 4. Conjugation
      if (conjugation) {
        backExtra += `<div style="margin-top:8px; border-top: 1px solid #eee; padding-top:8px;">${conjugation}</div>`;
      }
      ...
      if (posAbbr || meaning) {
        front += `<div style="color:#888;font-size:0.85em;margin-top:10px;"><b>${posTag}</b> ${meaning || ''}</div>`;
      }

      let back = `<div style="font-size:1.3em;margin-bottom:12px;color:#2a2a3a;"><b>${word}</b> ${phonetics || ''}</div>`;
      const rawMeaning = this.extractMeaningFromAI(rawAIText || definition, word);
      if (rawMeaning) back += `<div style="margin-bottom:12px;">${rawMeaning}</div>`;
      if (contextSentence) back += `<div style="color:#666;font-size:0.9em;"><i>${contextSentence}</i></div>`;
    ...
    if (screenshot) {
      ...
      const filename = `subtitle_${safeWord}_${Date.now()}.png`;
  ...
  async handleAddToAnki(word, context, container) {
    ...
    this.includeScreenshot = screenshotCb ? screenshotCb.checked : true;
  ...
  setupMessageListener() {
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      ...
      if (request.action === 'toggle') {
        ...
        const passedContext = request.context || null;
